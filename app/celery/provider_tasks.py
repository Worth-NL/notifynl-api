    ...
    try:
        ...
        if isinstance(e, SmsClientResponseException):
            current_app.logger.warning(
                "SMS notification delivery for id: %s failed",
                notification_id,
                exc_info=True,
    ...
    try:
        ...
        try:
            if isinstance(e, AwsSesClientThrottlingSendRateException):
                current_app.logger.warning(
                    "RETRY: Email notification %s was rate limited by SES",
                    notification_id,
                    extra={"notification_id": notification_id},
                )
            else:
                current_app.logger.exception(
                    "RETRY: Email notification %s failed", notification_id, extra={"notification_id": notification_id}
                )
...
def deliver_letter(self, notification_id):
    ...
    if notification.status != NOTIFICATION_CREATED:
        current_app.logger.warning(
            "deliver_letter task called for notification %s in status %s",
            notification_id,
            notification.status,
            extra={"notification_id": notification_id, "notification_status": notification.status},
        )
        return

    if notification.key_type != KEY_TYPE_NORMAL:
        current_app.logger.error(
            "deliver_letter task called for notification %s with key type %s",
            notification_id,
            notification.key_type,
            extra={"notification_id": notification_id, "api_key_type": notification.key_type},
        )
        return
    ...
    try:
        try:
            dvla_client.send_letter(
                ...
                client_reference=str(notification.client_reference),
        ...
        if isinstance(e, DvlaThrottlingException):
            current_app.logger.warning(
                "RETRY: Letter notification %s was rate limited by DVLA",
                notification_id,
                extra={"notification_id": notification_id},
            )
        else:
            current_app.logger.exception(
                "RETRY: Letter notification %s failed", notification_id, extra={"notification_id": notification_id}
            )
        ...
        update_letter_to_sending(notification)
        current_app.logger.warning(
            "Duplicate deliver_letter task called for notification %s",
            notification_id,
            extra={"notification_id": notification_id},
        )
    except Exception as e:
