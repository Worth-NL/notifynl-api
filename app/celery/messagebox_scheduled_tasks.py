...
def messagebox_process_unprocessed_messages():
    ...
    BerichtID == notification.id). Marks each envelope processed only after
    all its messages have been successfully enqueued -- at-least-once, safe
        ...
        for envelope_id in envelope_ids:
            try:
                ...
                current_app.logger.exception(
                    "Failed to fetch/parse messagebox envelope %s, will retry next poll", envelope_id
                )
                continue
            ...
            for bericht in batch.messages:
                try:
                    process_messagebox_client_response.apply_async(
                        [status, bericht.message_id, "ebms-adapter", bericht.process_code, bericht.stadium],
...
def check_if_messagebox_still_pending(max_minutes_ago_to_check: int = 60, max_hours_ago_to_check_sending: int = 24):
    ...
    messagebox_deliver (e.g. the trigger was lost); pending-virus-check needs
    human investigation, since a stuck scan isn't something safe to blindly
    retry.
